...
const builtins = {
  ...
  fn(...args) {
    ...
    return `async function ${name}(${params.join(', ')}) { ${params.map(param => `${param} = await ${param}`).join('; ')}; ${restCode.join(';\n')}${restCode.length ? '; ' : ''}return ${finalCode} }`
  }
}
...
function compileForm(atom) {
  ...
  if (fn in builtins) {
    ...
    code += 'require("infralang/lib/runtime").' + fn + '(' + args.map(compileAtom).join(', ') + ')'
  } else if (/^[\w\.]+$/.test(fn)) {
