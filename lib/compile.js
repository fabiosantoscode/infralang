...
`
    ...
    if (dependencies.length) {
      return `$runtime.dependencies(${JSON.stringify(dependencies)}, ${fnCode})`
    }
...
function compileForm (atom) {
  ...
  if (fn in builtinsNoCompile) {
    ...
    code += `$runtime.${fn}(${args.map(compileAtom).join(', ')})`
  } else if (/^[\w\.]+$/.test(fn)) {
...
module.exports = function compile (code, wrapper) {
  ...
  wholeCode = null
  const runtime = restProgram.concat([lastLine]).find(code => /\$runtime/.test(code))
