...
const UglifyES = require('@fabiosantoscode/uglify-es')
...
${indent(code)}
  ...
  code = wrap('const $runtime = ' + compile.getRuntime + '\nexports.func = ' + code)
  ...
  ;(function copyDir (zipDir, dir) {
    fs.readdirSync(dir).forEach(file => {
      ...
      if (stat.isFile()) {
        ...
        let contents = /\.js$/.test(file)
          ? UglifyES.minify(code, { mangle: true }).code
          : code
        ...
        if (/node_modules\/aws-sdk/.test(dir + '/' + file)) {
          if (/dist/.test(dir + '/' + file)) {
            return
          }
          if (/\.d\.ts/.test(file)) {
            return
        ...
        zip.folder(zipDir).file(file, contents)
      } else if (stat.isDirectory()) {
        if (file in package.devDependencies) {
          return
        }
        if (file in packageLock.dependencies && packageLock.dependencies[file].dev === true) {
          return
        }
        if (/node_modules\/aws-sdk/.test(dir + '/' + file)) {
          if (/dist/.test(dir + '/' + file)) {
        ...
        if (/\.git|sinon|\.changes|babel-runtime|core-js/.test(dir + '/' + file)) {
          return
        }
