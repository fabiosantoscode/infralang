...
function repeat(promiseGen, cb) {
  const onRes = res => {
    if (res) cb(res)
    return repeat(promiseGen, cb)
  }
  return promiseGen().then(onRes, () => onRes(null))
}
...
async function createFunction(name, code) {
  ...
  zip.file('code.js', 'exports.func = ' + code)
